...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        if (check || checkAcc || checkVerify) {
            ...
            //   { id: newPatient._id, email }, 'shhhhh',
            //   {
            //     expiresIn: "1h"
            //   })
...
router.get('/find/:uid', async (req, res) => {
    ...
    try {
      ...
      const pdata = await patient.findOne({ uid: id })
        .then((data) => {
...
router.post('/del/:uid', async (req, res) => {
    ...
    const list = await patient.findOne({ uid: uid })
    if (list) {
...
router.put(`/update/:uid`, async (req, res) => {
    try {
      ...
      const findData = await patient.findOne({ uid: uid })
